...
class FinanceAdvisorSystem:
    ...
    async def analyze_finances(self, financial_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            budget_prompt = self._create_budget_prompt(financial_data)
            budget_response = self.model.generate_content(budget_prompt)
            budget_analysis = self._parse_response(budget_response.text, BudgetAnalysis)
            
            # Prepare savings strategy based on budget analysis
            savings_prompt = self._create_savings_prompt(financial_data, budget_analysis)
            savings_response = self.model.generate_content(savings_prompt)
            savings_strategy = self._parse_response(savings_response.text, SavingsStrategy)
            
            # Prepare debt reduction plan
            debt_prompt = self._create_debt_prompt(financial_data, budget_analysis, savings_strategy)
            debt_response = self.model.generate_content(debt_prompt)
            debt_reduction = self._parse_response(debt_response.text, DebtReduction)
    ...
    def _create_budget_prompt(self, financial_data: Dict[str, Any]) -> str:
        expenses = financial_data.get("manual_expenses", {}) or {}
        transactions = financial_data.get("transactions", [])
        
        expense_text = ""
        if expenses:
            expense_text = "\n".join([f"- {k}: ${v:.2f}" for k, v in expenses.items() if v > 0])
        elif transactions:
            df = pd.DataFrame(transactions)
            expense_text = df.groupby('Category')['Amount'].sum().to_string()
        
        prompt = f"""You are a Budget Analysis Agent. Analyze the following financial data and provide budget analysis in JSON format.

Monthly Income: ${financial_data.get('monthly_income', 0):.2f}
Number of Dependants: {financial_data.get('dependants', 0)}
...
{{
    ...
    "recommendations": [
        {{"category": "<category>", "recommendation": "<advice>", "potential_savings": <number>}}
    ]
}}

Ensure percentages add up to 100% and include at least 3-5 specific, actionable recommendations with estimated savings."""
        
        return prompt
    
    def _create_savings_prompt(self, financial_data: Dict[str, Any], budget_analysis: Dict[str, Any]) -> str:
        prompt = f"""You are a Savings Strategy Agent. Based on the following budget analysis, create a savings strategy in JSON format.

Monthly Income: ${financial_data.get('monthly_income', 0):.2f}
Total Expenses: ${budget_analysis.get('total_expenses', 0):.2f}
...
{{
    ...
    "recommendations": [
        {{"category": "<name>", "amount": <number>, "rationale": "<reason>"}}
    ],
    "automation_techniques": [
        {{"name": "<technique>", "description": "<how_to>"}}
    ]
}}

Base emergency fund on 6 months of expenses. Include at least 3 savings recommendations."""
        
        return prompt
    
    def _create_debt_prompt(self, financial_data: Dict[str, Any], budget_analysis: Dict[str, Any], savings_strategy: Dict[str, Any]) -> str:
        debts = financial_data.get("debts", [])
        debts_text = "\n".join([f"- {d['name']}: ${d['amount']:.2f} at {d['interest_rate']}%" for d in debts])
        
        prompt = f"""You are a Debt Reduction Agent. Create an optimized debt payoff plan in JSON format.

Debts:
{debts_text}

Monthly Income: ${financial_data.get('monthly_income', 0):.2f}
Monthly Expenses: ${budget_analysis.get('total_expenses', 0):.2f}
...
{{
    ...
    "recommendations": [
        {{"title": "<title>", "description": "<details>", "impact": "<expected_impact>"}}
    ]
}}

Compare both Avalanche (highest interest first) and Snowball (smallest balance first) methods."""
        
        return prompt
    
    def _parse_response(self, response_text: str, schema_class) -> Dict[str, Any]:
